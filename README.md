...
# 3. 启动所有服务（默认会配置并启用 Nginx）
bash start-all.sh

# 4. 打开管理台，在“系统配置”页面填写：
#    LLM_API_KEY / LLM_BASE_URL / LLM_MODEL / SMTP_USERNAME / SMTP_PASSWORD
#    （SMTP_HOST 默认 smtp.qq.com，可不改）
...
> - 首次启动时如果 LLM/SMTP 还没填，服务依然会启动，但“生成摘要/发送邮件”会不可用；在前端保存配置后可直接重试，无需重启服务。
> - 你仍然可以手动编辑 `.env`，但不是必需步骤。
...
无论哪种方式，以下两项是**必填**的（可通过前端配置页保存到 `.env`）：
